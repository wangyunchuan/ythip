...
var inputName = '';
var inputValue = '';
class MaterialBarCodeMain extends Component {
    constructor(props) {
        super(props);
        this.state = {
            serverIp:'',
        ...
        if(barcode) {//正确扫描后有数据执行
            if( barcode !== 'nodata'){
                ...
                userInfo.AsyncGetUserInfo("usertoken", (userinfo) => {
                    ...
                    //通过制定URL获取数据
                    let url = serverIp+"/ythip/mobileController.do?getMaterialBarCodeInfo";
                    console.log(url);
    ...
    onPressCallback = () => {
        ...
        userInfo.AsyncGetUserInfo("usertoken", (userinfo) => {
            ...
            console.log("开单人[name]："+name)
            console.log("开单人工号[name]："+username)
...
export default class MaterialBarCode extends Component{
    render(){
        return(
            <Navigator
                initialRoute={{name:'MaterialBarCode',component:MaterialBarCodeMain}}
                renderScene={
            (route, navigator) =>
             {
